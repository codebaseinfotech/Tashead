...
class TBAddress : NSObject, NSCoding{

	var code : Int!
	var message : String!
	var result : [TBAddressResult]!
	var status : Bool!
	...
	@objc func parseJSONData(fromDictionary dictionary: NSDictionary)
	{
		...
		result = [TBAddressResult]()
		if let resultArray = dictionary["result"] as? [NSDictionary]{
			for dic in resultArray{
				let value = TBAddressResult(fromDictionary: dic)
				result.append(value)
			}
		}
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         result = aDecoder.decodeObject(forKey: "result") as? [TBAddressResult]
