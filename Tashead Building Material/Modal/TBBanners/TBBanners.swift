...
class TBBanners : NSObject, NSCoding{

	var code : Int!
	var message : String!
	var result : [TBBannersResult]!
	var status : Bool!
	...
	@objc func parseJSONData(fromDictionary dictionary: NSDictionary)
	{
		...
		result = [TBBannersResult]()
		if let resultArray = dictionary["result"] as? [NSDictionary]{
			for dic in resultArray{
				let value = TBBannersResult(fromDictionary: dic)
				result.append(value)
			}
		}
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         result = aDecoder.decodeObject(forKey: "result") as? [TBBannersResult]
