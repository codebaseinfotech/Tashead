...
class TBProduct : NSObject, NSCoding{

	var code : Int!
	var message : String!
	var result : [TBProductResult]!
	var status : Bool!
	...
	@objc func parseJSONData(fromDictionary dictionary: NSDictionary)
	{
		...
		result = [TBProductResult]()
		if let resultArray = dictionary["result"] as? [NSDictionary]{
			for dic in resultArray{
				let value = TBProductResult(fromDictionary: dic)
				result.append(value)
			}
		}
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         result = aDecoder.decodeObject(forKey: "result") as? [TBProductResult]
