...
class TBAreaList : NSObject, NSCoding{

	var code : Int!
	var message : String!
	var result : [TBAreaListResult]!
	var status : Bool!
	...
	@objc func parseJSONData(fromDictionary dictionary: NSDictionary)
	{
		...
		result = [TBAreaListResult]()
		if let resultArray = dictionary["result"] as? [NSDictionary]{
			for dic in resultArray{
				let value = TBAreaListResult(fromDictionary: dic)
				result.append(value)
			}
		}
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         result = aDecoder.decodeObject(forKey: "result") as? [TBAreaListResult]
