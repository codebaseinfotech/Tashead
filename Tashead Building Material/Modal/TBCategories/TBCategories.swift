...
class TBCategories : NSObject, NSCoding{

	var code : Int!
	var message : String!
	var result : [TBCategoriesResult]!
	var status : Bool!
	...
	@objc func parseJSONData(fromDictionary dictionary: NSDictionary)
	{
		...
		result = [TBCategoriesResult]()
		if let resultArray = dictionary["result"] as? [NSDictionary]{
			for dic in resultArray{
				let value = TBCategoriesResult(fromDictionary: dic)
				result.append(value)
			}
		}
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         result = aDecoder.decodeObject(forKey: "result") as? [TBCategoriesResult]
